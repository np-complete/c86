...
これも最後まで何をすれば回収できるのか判明しませんでした[^1]。
...
例えば砲撃の結果は、 `api_hougeki` の中に `api_def_list` と `api_damage` の2つの配列[^2]が入っています。
...
このデータを、`api_def_list` の `n` 番目に書かれた艦[^3]に `api_damage` の `n` 番目に書かれた数字をダメージとして与える、というルールで処理していきます。
...
元のレスポンスは `for` 文 という**コンピュータの処理の都合**に合わせて作られていて不適切です[^4]。
...
すでに出撃中の場合[^5]、APIは `400 Bad Request` を返して失敗します。

この**出撃のリソース**には何時でも、何回でもアクセスでき、常に同じ情報が取得できます[^6]。
...
リクエストを見るだけで `done` が `true` に設定されると想像できるでしょう[^7]。
...
移動できない場合[^8]は、`400 Bad Request` を返します。
...
これは**何度でも実行できる**べきです。
また、変更できない場合[^9]は、`400 Bad Request` を返してもいいし、
...
きちんと設計すれば直前の状況まで復旧することは可能です[^10]。
今回の実装例だと、過去も含めて全ての出撃を再現可能です[^11]。
...
これもただの一例なので、**よく考えて作ろう**とか**俺はこんなふうに思考して作ってる**って部分が伝わればいいと思います[^12]。
...
[^1]: オリョクル? なにそれ美味しいの?
[^2]: 誰が攻撃したかは戦闘結果に関係ないので無視している
[^3]: 敵味方艦混合でインデックスが振られる 味方艦:1-6 敵艦:7-12 みたいな感じに
[^4]: 元レスポンスの配列は固定長 つまり固定回数の `for` を回している可能性がある 圧倒的にダサい
[^5]: つまり `done` が `false` な `sotie` が存在する場合
[^6]: このような性質を**冪等性**と言う HTTPの `GET` は冪等性を持つべきとされている
[^7]: この方法を**論理削除**という 記録を残す必要がなければ単に**物理削除**してもよい
[^8]: 戦闘しないといけない場合や終端の場合など
[^9]: 少数編成の場合や通常戦闘でない場合
[^10]: 現状でも可能だと思うんですけどね・・・出撃終了条件を変な扱いしてるから・・・
[^11]: 出撃時の艦隊の状態を保存する必要があります
[^12]: こわいこわいRESTfulおじさんに見つかりませんように・・・
